...
async function showMainMenu(){
    ...
    */
    ...
    /*
    メトロノームのON/OFFを読み込みます(v177)。

    設定 ⚙️ →「🥁 メトロノーム」で選んだ状態を、次にアプリを開いた
    時にも覚えているようにするためです(上の2つと同じ考え方)。

    ⚠️ ここでは読み込むだけで、音は鳴り始めません。実際に刻み始める
       のは🕺ノリノリRun再生で曲が鳴った時です(js/metronome.js)。
    */
    await loadMetronomeSetting();
