...
function applyNoriRunPitch(){
    ...
    */
    ...
    ⚠️ 第3引数の true は「マイピッチには触るな」の合図です(v171)。

    この関数は **すでに決まっているマイピッチを反映するだけ** で、
    新しいテンポを決める場所ではありません。にもかかわらずv170では、
    この先でマイピッチが「その曲での見かけのBPM」に書き換えられて
    いました。曲の元テンポが極端に遅い/速いと、再生速度の安全装置
    (0.5〜2.0倍)に当たって別の値になってしまいます
    (詳しい経緯は js/pitch.js の applyTempo の中のコメント)。
    */
    applyTempo(noriRunMyPitch / base,false,true);
...
function exitNoriRunMode(){
    ...
    */
    ...
    このモードでは2枚のデッキを交互に使い、接続中は次の曲を裏で
    走らせています。メインメニューへ帰る時にそれを止めておかないと、
    **音量0のまま裏で鳴り続け、電池を食い続けます。**

    今この瞬間は裏でまだ何も鳴っていないことの方が多いのですが、
    clearIdleDeck()(js/deck.js)は何も載っていないデッキに対しては
    何も壊さないので、毎回呼んで構いません。「出る時は必ず片付ける」と
    決めておく方が、片付け漏れを探すより確実です。

    ⚠️ 主役のデッキ(いま鳴っている曲)には触れません。竹弘が曲を
       聴きながらメインメニューへ戻っても、曲は止まらずに続きます。

    v170から、片付けの前に cancelConnect() を呼んでいます。繋ぐ予約が
    残ったままメインメニューへ戻ると、**予約の時刻が来た瞬間に、
    メインメニューで聴いている曲が勝手に切り替わります。**
    */
    cancelConnect();
