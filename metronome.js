...
function canRingMetronome(){
    ...
    */
    if(!hasNoriBeatData(track)){ return false; }
...
function scheduleMetronomeClick(atCtxSec){
    ...
    /*
    「タタン」になる予約は、ここで見送ります(v201)。

    すぐ近く(半拍より近く)にもう音がいる時だけ true が返ります。
    その拍は先客がすでに鳴らしているので、**二度目を鳴らさない**のが
    正しい形です。詳しい理由は shouldSkipDoubleHit() のコメントに。

    ⚠️ 音を作る部品(createMetronomeClick 等)を作る**前**に置いています。
       後ろに置くと、鳴らさない音のために部品を作って捨てることになります。
    */
    if(shouldSkipDoubleHit(atCtxSec)){ return; }
...
/**
 * 「タタン」(2打)になる予約を見送ります(v201)。
 *
 * ------------------------------------------------------------
 * 【原因は実測で確定した(2026-09-20)】
 *
 * 竹弘の報告(2026-09-13):「ノリノリアシストが、たまにタタンと2打鳴る」。
 * v199で実測ログを仕込み、竹弘の実機で3件つかまえました。
 *
 *     間隔 30ms  (1拍 429ms)  接続の直後      先客 : 鳴り始め済み
 *     間隔 96ms  (1拍 429ms)  接続の直後      先客 : 鳴り始め済み
 *     間隔 128ms (1拍 429ms)  ⚠️ 接続ではない  先客 : 鳴り始め済み
 *     間隔 179ms (1拍 462ms)  ⚠️ 接続ではない  先客 : 鳴り始め済み
 *
 * **4件とも「鳴り始め済み」。** 見立てどおりでした。
 *
 * ⚠️ 3件目が大事です。近くに「★接続の瞬間」が無く、`playTrack開始` の
 *    直後でした。つまりタタンは接続だけの問題ではなく、**「拍を取り直す」
 *    場面すべて**(曲を手でタップした時など)で起きます。
 *
 * 【なぜ二度鳴るのか ―― 同じ拍を二度鳴らしている】
 *
 *     曲が繋がる瞬間、先行曲の13拍目と後続曲の0拍目は
 *     **設計上まったく同じ瞬間**です(それが「13=0」接続)。
 *
 *       ① 先行曲の格子で13拍目が鳴り始める                     ♪
 *       ② 再生位置が飛んだので clearScheduledClicks() が走る
 *          → ①はもう鳴り始めているので、鳴らしきらせる(v180)
 *       ③ 後続曲の格子で予約し直す → **0拍目**を予約する
 *       ④ ①と③は同じ拍なのに、数十ms差で二度鳴る               ♪
 *          → 「タタン」
 *
 * ------------------------------------------------------------
 * 【どう直すか ―― 見送っても拍は抜けない】
 *
 * 半拍より近いところに先客がいたら、**これから入れる予約を1つ見送ります。**
 *
 * ⚠️ 竹弘へ:「拍が1つ抜けるのでは?」と心配になるところですが、
 *    **抜けません。** 先客(すでに鳴っている音)が、その拍を担当して
 *    いるからです。見送るのは「二度目」の方だけです。
 *
 *     直す前   ♪♪ … … ♪ … ♪     ← タタン
 *     直した後 ♪  … … ♪ … ♪     ← 先客がその拍を鳴らしている
 *
 * ⚠️⚠️ **境目を「半拍」より狭くしてはいけません。**
 *
 *    半拍より近い音は「別の拍」ではあり得ません(ふつうの拍は1拍ぶん、
 *    マイピッチ170なら353ms離れている)。だから半拍が正しい境目です。
 *
 *    見送ると次の拍まで少し間延びするので、のりは一度「1/3拍まで
 *    狭めようか」と考えました。**それは間違いでした。**
 *    4件目の実測がこうだったためです:
 *
 *        間隔 179ms / 1拍 462ms
 *          半拍  = 231ms → 179ms は内側 ⭕ 見送れる
 *          1/3拍 = 154ms → 179ms は外側 ❌ 取りこぼす
 *
 *    **実測に合わせること。狭めると取りこぼします。**
 *
 * ⚠️ 見送っても、拍の数え方は進みます(呼び出し元の
 *    scheduleMetronomeBeats / scheduleMetronomeFreeRun が、予約できたか
 *    どうかに関わらず次の拍へ進めるため)。**格子はずれません。**
 *
 * ------------------------------------------------------------
 * 【ログ】
 *
 * 見送った時だけ1行出します。ふつうに鳴っている間は何も出ません。
 *
 *     ★ノリノリアシスト 2打を見送りました : 間隔 30ms (1拍 429ms)
 *       / 先客 : 鳴り始め済み
 *
 *     鳴り始め済み … 上の①〜④どおり(想定内。これが出れば直っている証拠)
 *     予約中       … ⚠️ 別の原因(同じ拍を二重に予約している)。
 *                    この文字が出たら、のりに知らせてください
 *
 * ⚠️ 竹弘の実機で「タタン」が再発しないと確認できたら、この関数と
 *    scheduleMetronomeClick() の中の呼び出し1行を削除してよい
 *    (CLAUDE.md の「本番リリース前に削除するもの」に登録済み)。
 *    ただし**接続はノリRunの心臓部**なので、接続ログと同じく
 *    しばらく残して実走で見張るのが安全です。
 *
 * @param  {number}  atCtxSec - これから予約する音の時刻(AudioContextの時計)
 * @return {boolean} true なら見送る(予約しない)
 */
function shouldSkipDoubleHit(atCtxSec){

    // 1拍の長さ(実秒)。js/connect.js の getBeatSec() を借ります
    const beatSec = getBeatSec();

    // 1拍の長さが分からない時は、判断できないので見送りません
    if(!isFinite(beatSec) || beatSec <= 0){ return false; }
    ...
    find は「条件に合った最初の1つを返す」命令です(見つからなければ
    undefined)。forEach と違って**見つかった時点で探すのをやめる**ので、
    1秒に何度も通るこの場所に向いています。
    */
    const neighbor = metronomeScheduled.find(function(entry){

        // Math.abs は「差の大きさ」だけを取り出す命令(前後どちらでも正の数になる)
        return Math.abs(atCtxSec - entry.atCtxSec) < beatSec / 2;

    });

    // 先客がいなければ、ふつうの拍なのでそのまま予約します
    if(!neighbor){ return false; }

    const gapSec = Math.abs(atCtxSec - neighbor.atCtxSec);

    const state = (neighbor.atCtxSec <= now) ? "鳴り始め済み" : "予約中";

    console.log(
        "★ノリノリアシスト 2打を見送りました : 間隔 " + Math.round(gapSec * 1000) + "ms" +
