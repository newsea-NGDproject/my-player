    ...
    */
    if(isNoriRunMode){
        ...
        */
        ...
        /*
        繋ぐ準備の最中なら、その計算をやり直します(v170)。

        【なぜ必要か】
        速さが変われば、接続点に着く時刻も変わります。予約したまま
        だと、早すぎる/遅すぎる場所で繋がってしまいます。

        裏で助走している曲も同じです。速さが変わると接続点までに
        進む距離が変わるので、**接続点でちょうど0拍目に来るように
        置き直します。**

        走りながら定規を動かすのは、まさに助走中にも起こりうる操作
        です。ここで追従しておかないと、繋ぎ目で必ずテンポが乱れます。

        ⚠️ 裏のデッキの速さも、この中で **その曲自身の元テンポ** から
           計算し直されます。曲ごとに元テンポが違うので、先行曲と同じ
           倍率を当ててはいけません(js/deck.js の deckTrackIds 参照)。

        繋ぐ準備をしていない時は、中で何もせずに戻ります。
        */
        rescheduleConnect();
