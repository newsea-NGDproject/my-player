...
function getTrackCover(track){
    ...
    cover_art_use が "original" なら、自作ジャケットを持っていても
    **元のジャケットを出します。**

    ⚠️⚠️ **v207では「旗は持たない方が安全」と書きましたが、撤回します。**

    竹弘の指摘(2026-09-21):

        「カメラ写真をセットした後、オリジナルに戻すと、カメラ写真を
          破棄してしまうので、(マイファイルで選択した写真)か
          (カメラ写真)に戻せる機能を追加したい」

    v209までは「オリジナルに戻す＝自作を削除する」という作りでした。
    旗が無いぶん食い違いは起きませんが、**戻した瞬間に自作ジャケが
    永久に失われます。** 苦労して撮った写真が消えるのは、食い違いより
    ずっと困ります。

    → 自作は消さずに残し、**どちらを見せるかだけ**を旗で切り替えます。

    ⚠️ 旗が "original" でも、**元のジャケットを持っていない曲**では
       意味がありません。その場合は下へ落ちて自作を返します
       (「&& track.cover_art」がその番人です)。
    */
    if(track.cover_art_use === "original" && track.cover_art){
        return track.cover_art;
    }

    /*
    旗が無い曲(v209までに作った曲、および自作していない曲)は、
    今までどおり「自作があればそちら、無ければ元」です。
    */
