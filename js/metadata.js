...
v206で 2 に上げた。下の COVER_ART_SIZE を 96 → 480 にしたので、
**竹弘がDBをクリアしなくても、起動時に369曲が自動で取り直される。**
(これが無いと「取得済み」の印が付いた曲は96pxのまま永久に残る)
*/
const COVER_ANALYZER_VERSION = 2;

/*
DBに保存するジャケット画像の一辺の長さ(ピクセル)。
**480 = 480×480ドットの正方形**(約23万画素)。

------------------------------------------------------------
【なぜ96pxから480pxに上げたのか(v206、竹弘の判断 2026-09-21)】

v204でジャケットの特大表示を作ったところ、竹弘の実機で
「画質はひどすぎた」となった。96px角を3〜4倍に引き伸ばして
いたので当然で、**保存サイズそのものを上げる必要があった。**

【480という数字の出し方】

このアプリは昔から「**表示サイズの2倍以上で保存する**」という
考え方で来ている(下の旧コメントの96pxも、44px表示の2倍を
超える値として選ばれた)。同じ理屈を特大表示に当てはめる:

    特大表示の一辺 = 上半分のジャケット(約76px) × 3倍 ≒ 228px
    スマホは高DPI(1pxを2〜3個の点で描く)なので2倍以上ほしい
    → 456px以上が理想 → きりの良い **480px**

⚠️ 竹弘の当初案は「一辺3倍(=288px)」だったが、それだと
   228pxの1.26倍しかなく、高DPIでは輪郭が甘くなる。
   **一辺3倍は「表示の大きさ」の話で、「保存の解像度」とは別**
   という点を確認した上で480pxに決めた。

【データ量について(竹弘に説明済み)】

一辺が5倍なら面積は25倍になる。ただしJPEGは圧縮が効くので、
実際のファイルは15〜20倍くらいに収まる見込み。

    96px角   1曲 数KB      → 369曲で 1〜2MB
    480px角  1曲 数十KB    → 369曲で 25〜30MB 前後

原寸(600×600〜1400×1400、1曲500KB〜1MB)を保存しない方針は
v72から変わらない(竹弘の判断、2026-08-08)。480pxは
「特大表示に耐える最小限」であって、原寸保存ではない。

⚠️ **画質の上限は、音楽ファイルに埋め込まれている画像で決まる。**
   埋め込みジャケが200px角しかない曲は、480pxで保存しても
   元が持っていない細かさは出てこない(引き伸ばしになる)。
------------------------------------------------------------
*/
const COVER_ART_SIZE = 480;
 ...
 *   ③ COVER_ART_SIZE 角のJPEGに縮小する(v206から480px)
