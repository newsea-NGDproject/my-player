...
/*
頭出し接続で、後続曲の音量を0から1へ上げるのにかける時間(秒)です(v202)。

【なぜ必要か ―― 竹弘の報告「頭出し接続でブチと鳴る」】

音の波は、上下に揺れながら進んでいます。無音(音量0)から**いきなり**
本来の大きさに切り替えると、その瞬間の波の値まで一気に飛ぶことになり、
スピーカーの紙が瞬間的に押し出されて「ブチ」と鳴ります。

    音量0 ─────┐
               │ ← ここが垂直の段差。これが「ブチ」の正体
               └──── 本来の大きさ(波の途中の値)

v201までは setDeckVolume(toDeck,1) で一瞬に上げていました
(js/deck.js の setDeckVolume は setValueAtTime で即座に変えます)。

⚠️ **これはv156でまったく同じ原因を直しています。** タップ補正で
   竹弘が報告した「押した瞬間にノイズがブッて入る時がある」がそれで、
   js/tap.js の TAP_FADE_SEC(0.015秒)で解決しました。
   **「時がある」の理由も同じ** ―― 始めた位置がたまたま波の底
   (振幅ゼロ付近)なら段差が無いので鳴りません。

【長さの決め方(v202は0.015秒 → v203で0.030秒)】

v202で15msを入れた時の竹弘の実機報告(2026-09-20):

    「ちょっと効果足りなかったようです。小さくなりました。
      ブチが、プッくらいになったので、もう少し長さが必要」

**対策の向きは合っていた**(ブチ→プッに小さくなった)ので、長さを
倍の30msにします。

⚠️ **ただし、長くすればいいというものではありません。** 竹弘の指摘
   「アタックで始まる曲もあるかもしれない」がそのとおりで、伸ばすほど
   曲の出だしの勢いが鈍ります。人がフェードインを「鈍った」と感じ
   始めるのは**50msを超えたあたり**から。30msはその手前で、段差を
   消すには十分な長さです。

⚠️ **30msでも残るなら、次は長さではなく「形」を変えます。**
   いまは音量をまっすぐ上げていますが、立ち上がりを緩やかな曲線に
   すると、同じ長さでもっと自然になります(buildCurve の power)。
   **長さを伸ばし続けるより副作用が少ない**ので、そちらを先に試すこと。

⚠️ **tap.js の TAP_FADE_SEC を借りずに、別の定数として持ちます。**
   値も考え方も同じですが、**用途が別**だからです(あちらはタップ補正
   画面で曲を試聴する時の設定)。借りてしまうと、タップ補正側の都合で
   数値を変えた時に、接続=ノリRunの心臓部が巻き込まれます。

⚠️ **この時間は「拍」で数えません。** ノリRunでは時間を必ず
   「拍数 × 1拍の長さ」で計算する決まりですが、それは**繋ぐ位置や
   長さ**の話です。これはスピーカーの段差を消すための設定なので、
   曲のテンポとは無関係に一定でなければいけません。
*/
const CONNECT_HEAD_FADE_SEC = 0.030;

/*
頭出し(currentTime を0に戻すこと)が終わるのを待つ、いちばん長い
時間(秒)です(v203)。

【なぜ待つ必要があるのか ―― v202で見落としていたこと】

無音の間、後続曲は**曲の途中**(実測では35秒あたり)を音量0で鳴り
続けています。無音が明ける瞬間に currentTime を0へ戻しますが、
**その命令はすぐには完了しません。**

    currentTime = 0 と命令する
      ↓ 数ms〜数十ms   ← この間、まだ「曲の途中」の音を出している
    頭出しが実際に完了する

v202では、この完了を待たずに音量を上げていました。つまりフェードで
上げている最中に、**曲の途中の一瞬が漏れていた**可能性があります。

⚠️ **これはフェードを長くしても消えません。** 長くするほど漏れる
   時間も伸びるためです。**待つことでしか消せません。**

【なぜ0.1秒で打ち切るのか】

曲の頭は、すでに読み込み済みの場所です(そこを通ってきたわけでは
ありませんが、ファイルの先頭は最初に読まれています)。だから頭出しは
ふつう数msで終わります。

それでも万一 `seeked` が来なかった時に**永久に無音のままになる**のが
いちばん困るので、0.1秒で見切りをつけて音量を上げます。
⚠️ 実際に何ms待ったかは🐛パネルに出ます。**いつも「時間切れ」と
   出るようなら、この考え方自体を見直すこと。**
*/
const CONNECT_HEAD_SEEK_WAIT_MAX_SEC = 0.1;
...
/**
 * いま頭出し接続の「無音の最中」かどうかを返します(v192)。
 *
 * js/metronome.js から呼ばれます。**曲は止まっていても、拍は続いている**
 * ということを、あちらに伝えるためのものです。
 *
 * 【なぜ必要か ―― 竹弘の実機報告(2026-09-06)】
 *
 *     『のりのりアシスト』中なのに、無音箇所だけ音がなくなってしまった。
 *     『のりのりアシスト』がないと、今回の機能がうまくいったか判断し
 *     にくい。だから、おもてなしモード同様に(略)曲と曲の間だけ無音と
 *     なるのは、『のりのりアシスト』機能をOFFにしている時だけ。
 *
 * 🧘(無音をはさんで繋ぐ)では先行曲を**音量0で完走**させるので、曲の
 * 位置は動き続けており、メトロノームはそのまま刻めていました。
 * 頭出し接続は先行曲を pause() するため(ended との競合を避けるため)、
 * 曲の位置が止まってしまい、拍を計算できなくなっていました。
 */
function isConnectSilence(){

    return !!(connectState && connectState.headStyle);
...
function doHeadConnect(){
    ...
    */
    ...
    /*
    ---- 音量を上げます(v202で「ブチ」対策、v203で作り直し) ----

    この繋ぎ方はクロスフェードしません(相手がもういないため)。
    ただし**一瞬で上げると「ブチ」と鳴る**ので、上げ方に手をかけます。

    ⚠️ v201までは setDeckVolume(toDeck,1) でした。あれは
       setValueAtTime で即座に切り替えるので、波形に垂直の段差が
       できていました(v156でタップ補正が踏んだのと同じ穴)。

    ⚠️ **v202では「待たずに」上げていました。それが残りの音の原因
       かもしれません。** すぐ上の currentTime = 0 はまだ完了して
       いない可能性があり、その間は**曲の途中の音**が出ています。
       v203では、頭出しが終わってから上げます(下の関数)。

    ⚠️ **繋ぐタイミングの計算には触れていません。** 鳴り始める時刻は
       v190のまま。変えたのは音量の上げ方だけです。
    */
    raiseHeadConnectVolume(toDeck);
...
/**
 * 頭出しが終わってから、後続曲の音量を上げます(v203)。
 *
 * ------------------------------------------------------------
 * 【なぜ「終わってから」なのか】
 *
 * 無音の間、後続曲は**曲の途中**を音量0で鳴り続けています。
 * 無音明けに currentTime を0へ戻しますが、その命令は**すぐには
 * 完了しません。** 完了を待たずに音量を上げると、フェードの最中に
 * **曲の途中の一瞬**が漏れて聞こえます。
 *
 *     v202(待たない)  頭出し命令 →〜〜〜♪ 曲の途中が漏れる → 曲の頭
 *     v203(待つ)      頭出し命令 →(無音のまま待つ)→ 曲の頭 ♪
 *
 * 竹弘の実機報告(2026-09-20)「ブチが、プッくらいになった」の、
 * **残った「プッ」の正体がこれではないか**という見立てです。
 *
 * ⚠️ これはフェードを長くしても消えません(長くするほど漏れる時間も
 *    伸びるため)。**待つことでしか消せません。**
 *
 * ------------------------------------------------------------
 * 【3つの入口があります】
 *
 *     ① もう終わっていた … seeking が false。待たずに上げる
 *     ② seeked が届いた  … ふつうはここ。数msで来るはず
 *     ③ 時間切れ         … 0.1秒待っても来ない時の保険
 *
 * ③が無いと、万一 seeked が来なかった時に**永久に無音のまま**に
 * なります。走行中の無音がいちばん困る失敗なので、必ず逃げ道を
 * 用意しておきます。
 *
 * ⚠️ `raised` という旗で「1回だけ」に絞っています。②と③が両方
 *    走っても、音量を2回上げることはありません。
 *
 * ------------------------------------------------------------
 * 【ログ(🐛パネル)】
 *
 *     頭出しの音量を上げました : 待ち 3ms(頭出し完了) / フェード 30ms
 *
 * ⚠️ **いつも「時間切れ」と出るようなら、この考え方自体が誤り**
 *    なので、見直してください。ふつうは「頭出し完了」で、待ちは
 *    数ms〜数十msのはずです。
 *
 * @param {HTMLAudioElement} deck - これから聞こえ始める後続曲のデッキ
 */
function raiseHeadConnectVolume(deck){

    // performance.now() は「ページを開いてから何ミリ秒か」を返します
    ...
    const raise = function(why){
        ...
        console.log(
            "頭出しの音量を上げました : 待ち " +
            (performance.now() - startedAtMs).toFixed(0) + "ms(" + why + ")" +
            " / フェード " + Math.round(CONNECT_HEAD_FADE_SEC * 1000) + "ms"
        );
    ...
    /*
    ① もう頭出しが終わっているなら、待つ必要はありません。

    seeking は「いま位置を動かしている最中かどうか」を表す、
    <audio> が最初から持っている印です。ここを見ずに seeked だけを
    待つと、**すでに終わっていた時にイベントが二度と来ず**、
    ③の時間切れまで0.1秒も無音になってしまいます。
    */
    if(!deck.seeking){
    ...
    },CONNECT_HEAD_SEEK_WAIT_MAX_SEC * 1000);
...
function doConnect(){
    ...
    */
    ...
    /*
    ---- 先行曲が聞こえなくなったら、重い計算を止める(v200) ----

    先行曲はこの後も音量0のまま完走します(止めるとOSに再生終了と
    見なされ、画面ロック中に次が鳴らなくなるため)。ただし**聞こえなく
    なった後まで、音程維持(WSOLA)を全力で回す必要はありません。**

    待つ時間は繋ぎ方で変わります。

        クロスフェード … 入れ替えが終わるまでは先行曲も聞こえている
                         → フェードの長さ + 1拍ぶんの余裕を待つ
        無音をはさむ   … フェードアウトは接続点より前に終わっている
                         → もう聞こえていないので 0(今すぐ)

    ⚠️ 余裕の1拍は、**秒で決め打ちせず拍で数えます**(竹弘の指示で、
       時間はすべて「拍数 × 1拍の長さ」で計算する決まりのため)。
       setTimeout は遅れることはあっても早まらないので、この余裕が
       あれば「音量が0になりきる前に音程が変わる」ことは起きません。

    ⚠️ 呼ぶのは if/else の**外**で1回だけです。両方の分岐の中に
       書くと、片方だけ直す事故が起きます(findWrapAroundTrackId を
       切り出した時と同じ理由)。

    詳しい経緯は scheduleSilentFromDeck() のコメントにあります。
    */
    scheduleSilentFromDeck(
        fromDeck,
        (silenceBeats === 0) ? (fadeSec + beatSec) : 0
    );
...
/**
 * 音量0になった先行曲の、重い計算(WSOLA)を止めます(v200)。
 *
 * ------------------------------------------------------------
 * 【v176の対策が、半分しか当たっていなかった】
 *
 * v176で「音量0で助走中の裏デッキは、音程維持(preservesPitch)を
 * 切る」という対策を入れました。聞こえない音のためにWSOLAという重い
 * 計算を回すのが無駄だったためです(js/deck.js の deckSilentPreRoll)。
 *
 * ところがその旗を立てていたのは**助走中(接続点より前)だけ**でした。
 * 接続点から先、先行曲は音量0のまま最後まで鳴り続けますが、そちらは
 * 手つかずで、**ずっとWSOLAを全力で回したまま**だったのです。
 *
 *     助走中(15秒)     … 後続曲のWSOLAを切る   ← v176で対策済み
 *     クロスフェード中   … 2曲ともWSOLA          ← 両方聞こえるので正しい
 *     そのあと完走まで   … 2曲ともWSOLA          ← ★ここが無駄だった
 *
 * 竹弘の実機ログ(2026-09-15)で実測した、その「無駄な時間」:
 *
 *     空の青さ→祝福          15秒(うち聞こえているのは6.86秒)
 *     見たこともない→花びら  18秒(同上)
 *     花びら→群青            16秒(同上)
 *
 * つまり**毎回8〜11秒、誰にも聞こえない音のために重い計算**を
 * していました。しかも先行曲と後続曲は再生速度が違う(実測で1.30倍と
 * 0.82倍)ので、2つとも別々に全力で回っていたことになります。
 *
 * 竹弘が報告した「音切れ」4件のうち3件が、まさにこの接続点でした。
 * WSOLAの計算が間に合わないと音が乱れる・途切れるためです。
 *
 * ------------------------------------------------------------
 * 【⚠️ なぜ「止める」のではなく「音程維持を切る」のか】
 *
 * いちばん軽くなるのは先行曲を pause() することですが、**それは
 * やってはいけません。** 音が途切れるとOSから見て「再生が終わった」
 * ことになり、画面ロック中に次の曲が鳴らなくなります(v144の教訓)。
 *
 * 音は鳴らしたまま、計算の中身だけを軽くします。音量0なので、
 * 音程が変わっても誰にも聞こえません。
 *
 * ------------------------------------------------------------
 * 【⚠️ 2つの見張り】
 *
 * 目を覚ました時、状況が変わっていることがあります。
 *
 *   ① 回数券(connectGeneration)… 竹弘が途中で曲を選び直すと
 *      cancelConnect() が番号を進めるので、古い予約は引き返します
 *   ② 主役に戻っていないか … 竹弘がクロスフェード中に曲をタップ
 *      すると、このデッキが主役として鳴り出していることがあります。
 *      そこで旗を立てると**聞こえている音の音程が狂います**
 *
 * @param {HTMLAudioElement} fromDeck - 音量0で完走していく先行曲
 * @param {number}           delaySec - 何秒後に切るか(0以下なら今すぐ)
 */
function scheduleSilentFromDeck(fromDeck,delaySec){

    if(delaySec <= 0){

        // 無音モード。先行曲はもう音量0なので、待たずに切ります
        setDeckSilentPreRoll(fromDeck,true);
    ...
    setTimeout(function(){
        ...
        // ② 主役に戻っていたら、聞こえているので切りません
        if(fromDeck === audioPlayer){ return; }
