...
まず「ノリ注入済み(🕺)か、まだか(🛌)」で分け、
注入済みの中では注入日時の新しい順に並べます。

つまり
    さっき仕込んだ曲 → 前に仕込んだ曲 → まだの曲
という並びになり、走る前に最後の仕上げをした曲がすぐ見つかります。
*/
function compareByNori(trackA,trackB){

    // 注入済みを 0、まだの曲を 1 として、注入済みを先に持ってきます
    const injectedA = trackA.is_analyzed ? 0 : 1;
    const injectedB = trackB.is_analyzed ? 0 : 1;
