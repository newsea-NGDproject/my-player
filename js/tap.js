...
function nudgeTapPosition(deltaSec){
    ...
    /*
    A地点だけ、曲の頭に寄せすぎないよう下限を設けます(v184)。

    ここまで詰めると、その曲へ繋ぐための助走が確保できなくなるためです
    (理由は TAP_POS_A_MIN_SEC のコメント)。

    ⚠️ B地点は 0 のままにします。B地点は「前へ戻す」のが本命の使い方で、
       戻せる範囲を狭めると、終盤でリズムが崩れる曲から逃げられなく
       なるためです。
    */
    const minSec = (tapState.phase === TAP_PHASE_A) ? TAP_POS_A_MIN_SEC : 0;
