...
【★実測(竹弘の実機、2026-09-20。21回ぶん)】

    5 / 10 / 11 / 11 / 11 / 12 / 14 / 14 / 14 / 14 / 14 / 15 / 15
    15 / 15 / 16 / 16 / 16 / 16 / 16 / 16 ms   ← 全件「頭出し完了」

**「時間切れ」も「待ち不要」も0件。** 頭出しには本当に10〜16msかかって
いました。そして ――

    v202のフェード     15ms
    頭出しの待ち時間   14ms(平均)   ← ほぼ同じ長さ!

つまりv202は、**フェードで音量を上げている区間のほぼ全部で、曲の
途中の音が漏れていた**ことになります。竹弘の報告「小さくなったが
プッが残る」「もう少し長さが必要」は、**長さの問題に見えていただけで、
実は待ちの問題**でした。v203で待つようにしたら消えました。

⚠️ **この教訓は他でも効きます**:「音がおかしい」時、音量の形だけを
   疑わないこと。**その音がどこから出ているか(位置が正しいか)**を
   先に確かめる方が早いことがあります。
*/
...
function canStartConnect(){
    ...
    /*
    ⚠️⚠️ **loop属性が効いている間は繋ぎません(v219で条件を変更)。**

    v218までは「1曲リピートなら問答無用で繋がない」でした。
    v219から、🕺ノリノリRun再生の1曲リピートは**接続で繋いで**
    ループさせます(竹弘の要望、棚③⑫)。

    【なぜ loop属性 を見るのか】

    loop属性 と 接続 は**どちらか一方しか動いてはいけません。**

        loop属性 … 曲が終わるとブラウザが勝手に頭へ戻す
        接続     … 終わる15秒前から裏で助走し、接続点で入れ替える

    両方が効くと、**裏で助走している最中に表がループして頭へ戻り**、
    拍も曲順もぐちゃぐちゃになります。

    そこで「loop属性が立っているなら繋がない」という形にしました。
    どちらを使うかは shouldLoopByConnect()(下)が1か所で決めていて、
    その結果が loop属性 に入ります。**ここはその結果を見るだけ**なので、
    二重になりようがありません。

    ⚠️ 上の行で「🕺ノリノリRun再生でなければ繋がない」と弾いているので、
       ここへ来る時点でメインメニューではありません。メインメニューの
       1曲リピートは今までどおり loop属性 のままです(v144の画面ロック
       対策を崩さないため)。
    */
    if(audioPlayer.loop){ return false; }
...
/**
 * 1曲リピートを「接続で繋いで」ループさせてよいかを返します(v219)。
 *
 * 竹弘の要望(棚③⑫):
 *     「🕺ノリノリRunの1曲ループで接続再生」
 *
 ...
 *
 *     loop属性 … 曲が終わるとブラウザが勝手に頭へ戻す。
 *                **JSが一切関与しない**ので、画面ロック中でも
 *                絶対に止まらない(v144。竹弘との実機調査で判明)。
 *                ただし**頭に戻る瞬間に拍が途切れる。**
 *
 *     接続     … 終わる15秒前から裏のデッキで同じ曲を助走させ、
 *                接続点(13拍目=0拍目)で入れ替える。
 *                **拍が途切れない。** ノリRunの本来の繋ぎ方。
 *
 * 🕺ノリノリRun再生では接続を使います。ここは走るための画面で、
 * **ループの継ぎ目で足が乱れるのがいちばん困る**からです。
 *
 * ⚠️ **メインメニューでは今までどおり loop属性 のまま**です。
 *    あちらは未注入曲(🛌)も鳴らす画面で、接続点を持たない曲が
 *    あります。v144の画面ロック対策も崩したくありません。
 *
 * ⚠️⚠️ **繋げない曲では false を返します。** 🕺の曲一覧には注入済み
 *    しか並びませんが、メインメニューから未注入の曲を持ち込んだ
 *    直後だけ、接続点を持たない曲が鳴っていることがあります。
 *    その時に loop属性 まで外すと、**曲が終わっても誰も次を鳴らさず
 *    無音になる**恐れがあります。**走行中の無音がいちばん困る失敗**
 *    なので、繋げないと分かっている時は loop属性 に任せます。
 *
 * ⚠️ **この関数が「どちらを使うか」を決める唯一の場所**です。
 *    呼ぶ側(js/player.js と js/queue.js)は結果を loop属性 に入れる
 *    だけ、canStartConnect() はその loop属性 を見るだけ。こうして
 *    おけば、**両方が同時に効く**という事故が起こりえません。
 * ------------------------------------------------------------
 *
 * @param  {Object}  track … これから鳴らす(いま鳴っている)曲
 * @return {boolean} 接続でループさせるなら true
 */
function shouldLoopByConnect(track){

    if(!isNoriRunMode){ return false; }

    if(currentPlayMode !== PLAY_MODE_ONE){ return false; }

    return canConnectTrack(track);
