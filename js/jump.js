...
【このファイルの役割】
 ...
 369曲もある曲一覧を、一気に移動するための操作です。
   ...
   指を離す         … その場所へ飛ぶ
     ...
     → 真ん中へ
     ↓ 一番下へ
     🔍 検索モードへ(v140で追加。js/search.js が担当)
...
【左方向(検索)について、2026-08-16→2026-08-23の経緯】

 当初(2026-08-16)は左方向に検索を足す案がありましたが、竹弘の
 判断で一旦取りやめていました。

    「検索は基本機能のプレイヤーを逸脱した大きなプログラムに
      なりそうなので、一旦辞めます」

 プレイヤーとしての基本機能がひととおり形になった2026-08-23、
 竹弘の判断で改めて追加することになりました。ジャンプ(移動)と
 いうより検索の性格が強いため、行き先の名前は "search" にして
 います。実際の検索モードの中身(検索欄の表示・絞り込み)は
 js/search.js が担当し、このファイルは「左にフリックされたら
 search.js を呼ぶ」入口の役目だけを持ちます。
 ...
 *
 * @param {string} action - "top" / "mid" / "end" / "search"
 */
function jumpTo(action){

    /*
    「検索」だけは曲一覧のスクロールではなく、検索モードを開く
    別の動きです(v140)。中身は js/search.js に任せ、ここでは
    呼び出すだけにします。
    */
    if(action === "search"){
