...
耳には一瞬で、「曲が遅れて始まった」とは感じません。それでいて
波形をなめらかに繋ぐには十分な長さです。
...
*/
const CONNECT_HEAD_FADE_SEC = 0.015;
...
function doHeadConnect(){
    ...
    */
    ...
    ただし**一瞬で上げると「ブチ」と鳴る**ので、ごく短い時間をかけて
    上げます。理由は CONNECT_HEAD_FADE_SEC のコメントに詳しく。

    ⚠️ v201までは setDeckVolume(toDeck,1) でした。あれは
       setValueAtTime で即座に切り替えるので、波形に垂直の段差が
       できていました(v156でタップ補正が踏んだのと同じ穴)。

    ⚠️ **この15msが「ひと呼吸」の役も果たします。** すぐ上で
       currentTime を0に戻したばかりで、曲の頭へ飛ぶ処理はまだ
       終わっていないかもしれません。立ち上がりの数msは音量がほぼ0
       なので、その間に落ち着きます。
       ⚠️ `seeked` を待つ形にはしていません。**待つと、その分だけ
          曲の頭が欠けます**(この瞬間は0拍目にぴたり合わせてある)。

    ⚠️ **繋ぐタイミングの計算には触れていません。** 鳴り始める時刻は
       v190のまま。変えたのは音量の上げ方だけです。
    */
    startFade(toDeck,0,1,CONNECT_HEAD_FADE_SEC);
