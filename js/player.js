...
async function playTrack(trackId){
    ...
    */
    ...
    ⚠️⚠️ **cancelConnect() より前に呼ぶこと(v175で順序を直しました)。**

       v174では下の cancelConnect() の方が先でした。すると回路がまだ
       無い状態で音量を調整することになり、その値が <audio>.volume に
       書き込まれます。直後に回路ができると、以後の音量は GainNode が
       持つため、**<audio>.volume に残った0を誰も1に戻せなくなります。**

       結果、デッキBだけ「シークは動くのに音が出ない」という状態に
       なりました(竹弘の実機報告、2026-09-02)。回路を先に用意して
       おけば、音量の調整は最初から GainNode に入ります。
