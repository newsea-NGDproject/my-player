...
const SETTINGS_DEFINITIONS = [
    ...
    },
    {
        key: "metronome",
        /*
        🏃 にしたのは竹弘の指示(2026-09-05)。
        「この機能はランナーのピッチをアシストする為の機能としたいので」
        ―― 楽器(🥁)ではなく、走ることを表す記号にしています。
        */
        icon: "🏃",
        label: "ノリノリアシスト"
    },
...
/**
 * ノリノリアシストの設定を、今開いてよいかを返します(v217)。
 *
 * 竹弘の指定(2026-09-21):
 *     「未注入曲🛌の時に(設定ボタンを)押せなくする」
 *
 * 【なぜ塞ぐのか】
 *
 * ノリ注入(タップ補正)が済んでいない曲は、拍がどこにあるか
 * 分かりません。その間にノリノリアシストをONにしても**何も
 * 起きない**ので、「押したのに鳴らない、壊れているのでは?」と
 * 思わせてしまいます。押せないようにして、理由を見せる方が親切です。
 *
 * ⚠️ **曲が鳴っていない時は塞ぎません。** 走る前に設定を整える
 *    場面まで塞いでしまうと、かえって不便になるためです。
 *
 * ⚠️ 判断そのものは js/metronome.js の hasNoriBeatData() に任せます。
 *    **同じ条件をここにも書くと、片方だけ直した時に「設定は開けるのに
 *    鳴らない」という食い違いが起きます。**
 */
function canOpenMetronomeSettings(){
 ...
 * メニューの中身を作り直します。
 *
 * ⚠️ **開くたびに呼ばれます**(openSettingsMenu の中)。だから
 *    「今そのとき押せるか」をここで決めれば、鳴っている曲が
 *    変わっても常に正しい状態で出ます。
 */
function renderSettingsMenu(){
    ...
    SETTINGS_DEFINITIONS.forEach(function(definition){
        ...
        今のところ、塞ぐのはノリノリアシストだけです。
        他の項目(繋ぎ方・ライセンス)は、どんな曲が鳴っていても
        関係なく開けます。
        */
        const blocked =
            (definition.key === "metronome") && !canOpenMetronomeSettings();

        /*
        3つ目のspanは、並び替えメニューとCSSを共有するための空欄です
        (項目の横幅が、記号・文字・余白の3つで決まっています)。
        塞いでいる時は、ここに理由を出します ―― **押せないだけで
        理由が無いと、壊れていると思われる**ためです。
        */
        const noteHtml = blocked
            ? "<span class='settings-note'>🛌 ノリ注入前</span>"
            : "<span></span>";
        ...
        if(blocked){
            ...
            /*
            disabled は「押せません」という標準の印です。見た目は
            c014.html の :disabled のCSS(薄くする)が担当します。
            */
            item.disabled = true;
...
// ==========================================================
// 3-3. ノリノリアシスト(v177 / v178で改名・音色を追加)
// ==========================================================
/*
🕺ノリノリRun再生で、マイピッチに合わせて音を鳴らすかを選びます。

【名前について】
竹弘の命名(2026-09-05):「この機能を一旦『ノリノリアシスト』と
